...
for index, row in top_5_mvp_hero_pairs.iterrows():
    ...
    for item in json_data["hero_name"]:
        if hero_code in item:
            ...
            print(f"{value} ({calculate_mvp_percentage(mvp, result_battle)}%)")
            break
...
sorted_first_pick_df = filtered_hero_sum.sort_values(by="first_pick", ascending=False)
top_5_first_pick_hero_pairs = sorted_first_pick_df[["hero_code", "first_pick"]].head(5)
